...
r=287.05
c=1005
lambda_wall =  0.17 / 0.25#0.01#0.17 / 0.25#5#0.3/0.25
# wall: 0.01 zeby sciany mialy ~~15C gdy zewn=0, 0.05 - wtedy sciany maja 8C
lambda_window = 0.96 / 0.005#0.96#96#0.96 / 0.005#2#0.005 #1.1/0.005  #1.1# nie moge mnozyc przez grubosc bo wtedy wieksze i bardziej mrozace jest wall
...
#alfa = 0.0019 # 0.0019 - wciaz jest slaba dyfuzja 0.1 100 ale moze to ok bo maly czas?
...
### kolejnosc kron?
BsW = - (np.kron(id_Ny, D1F(Nx)) / hx) + lambda_wall/lambda_air * Id # tu i nizej minu czy bez?
...
def f(grzejnik, u, ust_grzalki):  # albo room zamiast x,y? indeksy pokoju
  ...
  if np.mean(u[ind_wnetrza]) <= Si: #ind_wnetrza dodac do funkcji?
    # tylko tam gdzie grzejnik robic to dzialanie
    u_n[grzejnik] = u[grzejnik] * nu#u[grzejnik] * nu
  return u_n
print( P * r / (p * 16 * c))
...
for _ in range(t):
  # równanie du/dt
  u_current += ht * f(ind_grzejnik, u_current, 4)
  #u_current[ind_grzejnik] += ht * f(x, u_current)
...
levels0 = np.linspace(0,30,50)#u_0.min(), u_0.max(), 50)
'''
...
im1 = axs[0].contourf(X, Y, u_0.reshape(Nx, Ny), levels=levels0, cmap='viridis') #? .reshape(Ny, Nx)
...
print(u_current)
cp = plt.contourf(X, Y, u_current.reshape(Nx, Ny), levels=20, vmax=40, cmap='jet')
...
# - i okno i ściany są rachunkowo "zaokrąglone" do dirichleta?
